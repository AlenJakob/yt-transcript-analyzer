...
export default function TranscriptViewer({ segments, videoId }: TranscriptViewerProps) {
  ...
  const handleGenAi = async () => {
    ...
    try {
      ...
      const resp = await fetch('/api/ai', {
        ...
        body: JSON.stringify({
          transcriptText,
          promptPreset: 'Przeanalizuj poniższą transkrypcję i stwórz streszczenie. Zbierz najważniejsze informacje, nie pomijaj istotnych szczegółów',
        }),
      });
  ...
  return (
    <Box sx={{ mb: 5 }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ alignItems: 'center' }}>
          ...
          {viewMode === 'timestamps' && (
            <FormControl size="small" sx={{ minWidth: 170 }}>
              <InputLabel id="group-interval-label" sx={{ color: 'text.secondary', fontSize: '0.85rem' }}>
                Łączenie czanków
              </InputLabel>
              <Select
      ...
      {searchQuery && (
        <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', mb: 2, ml: 1 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          >
            <>
              ...
              <div>{isAiLoading ? 'Generuje odpowiedź...' : ''}</div>
            </>
          </Typography>
        </Box>
      </Paper>
              ...
              <Grid size={{ xs: 12, md: groupInterval > 0 ? 6 : 4 }} key={idx}>
                ...
                >
                  <CardContent sx={{ p: 2.5, '&:last-child': { pb: 2.5 } }}>
                    ...
                    <Typography variant="body2" sx={{ color: 'text.primary', lineHeight: 1.6, letterSpacing: '0.01em' }}>
                      {item.text}
                    </Typography>
                  </CardContent>
                </Card>
